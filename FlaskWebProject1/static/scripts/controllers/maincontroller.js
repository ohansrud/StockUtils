function MainController($scope ) {
    $scope.title = "TEL.OL";
    $scope.loading = false;
    $scope.chartdata = [[1426204800000,10, 15, 8, 9]];
    ...
    $scope.an = {};
    $scope.annotateStop = function (e){
        $scope.an = e;
        $scope.$apply();
        console.log(e);
    };

    $scope.annotateStep = function (e){
        $scope.anno = e;
        $scope.$apply();
        console.log(e);
        console.log(this);
        this.options.xValueEnd = e.screenX;
        this.options.yValueEnd = e.screenY;
        //this.shape.d['M', 0,0, 'L', e.screenX, e.screenY];
    };

    $scope.annotateClick = function (e){
        console.log(e);
    };

    $scope.getall = function(title){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.result, function (e, o) {
                    ...
                    $scope.chartConfig.series[0].id = title;
                    $scope.chartConfig.series[0].name = title;
    ...
    $scope.newAnnotation = function(e){
        if($scope.xValue == 0){
            $scope.xValue = e.xAxis[0].value;
            $scope.yValue = e.yAxis[0].value;
        }else{
        $scope.annotations.push(
        {
            linkedTo: 'TEL.OL',
            xValue: $scope.xValue,
            yValue: $scope.yValue,
            xValueEnd: e.xAxis[0].value,
            yValueEnd: e.yAxis[0].value,
            shape: {
                type: 'path'
            }
        });
